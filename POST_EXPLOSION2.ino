...
const long STEER_LIMIT_POS = 1000;
const long STEER_LIMIT_NEG = -1000;

// -------------------- Brake debug safety --------------------
// Keep this short for now so the actuator cannot sit stalled forever.
const unsigned long BRAKE_MAX_DRIVE_MS = 500;

// If true, pressing brake command extends the actuator.
// Leave this true since you said extending on press is desired.
...
BrakeMode brakeMode = BRAKE_MODE_STOP;
unsigned long brakeDriveStartMs = 0;
...
void setBrakeMode(BrakeMode newMode) {
  if (newMode != brakeMode) {
    brakeMode = newMode;
    brakeDriveStartMs = millis();
  }
}
...
void handleSerial() {
  ...
  if (line == "HELP") {
    ...
    Serial.println("");
    Serial.println("Brake debug commands:");
    Serial.println("B+  = brake apply/extend");
    Serial.println("B-  = brake release/retract");
    Serial.println("B0  = brake stop");
    return;
  }

  if (line == "STEER_ZERO") {
    long sc = readSteerCounts();
    steerZeroOffset = sc;
  ...
  if (line == "B+") {
    setBrakeMode(BRAKE_MODE_APPLY);
    Serial.println("OK: Brake apply/extend command");
    return;
  }

  if (line == "B-") {
    setBrakeMode(BRAKE_MODE_RELEASE);
    Serial.println("OK: Brake release/retract command");
    return;
  }

  if (line == "B0") {
    setBrakeMode(BRAKE_MODE_STOP);
    Serial.println("OK: Brake stop command");
    return;
  }
...
void homeSteeringToCenter() {
  ...
  // 2. Set zero at fully-extended position
  noInterrupts();
  steerCounts = 0;
  interrupts();
  Serial.println("Homing: zero set at extension.");

  // 3. Retract to center target
...
// -------------------- Brake runtime handler --------------------
void serviceBrake() {
  if (brakeMode == BRAKE_MODE_STOP) {
    brakeStop();
    return;
  }

  if (millis() - brakeDriveStartMs >= BRAKE_MAX_DRIVE_MS) {
    brakeStop();
    brakeMode = BRAKE_MODE_STOP;
    Serial.println("Brake timeout -> stop");
    return;
  }

  if (brakeMode == BRAKE_MODE_APPLY) {
    brakeApplyRaw();
  } else if (brakeMode == BRAKE_MODE_RELEASE) {
    brakeReleaseRaw();
  } else {
    brakeStop();
  }
...
void setup() {
  ...
  Serial.println("READY: SIMPLE BRAKE DEBUG BUILD");
...
void loop() {
  ...
  // Not armed or not homed -> everything safe
  if (!homingDone || !armed) {
    forceSafeOutputs();
  } else {
    // ---------------- Steering ----------------
    ...
    // ---------------- Brake ----------------
    // Priority: brake button > throttle > stop

    if (cmdBrake) {
      // Extend / apply brake
      setBrakeMode(BRAKE_MODE_APPLY);
    }
    else if (cmdThrottle > 100) {
      // Use throttle slider as retract
      setBrakeMode(BRAKE_MODE_RELEASE);
    }
    else {
      // No input → stop
  ...
  if (now - lastPrintMs >= PRINT_MS) {
    ...
    Serial.print(" HOMED=");       Serial.print(homingDone);
    ...
    Serial.print(" brakeMode=");   Serial.print((int)brakeMode);
