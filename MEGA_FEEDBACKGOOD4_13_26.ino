...
int brakeTarget = -1;      // -1 means no target active
bool brakeParkingMode = false;
bool cartInTransit = false;
...
void handleSerial() {
  ...
  if (line == "UNPARK") {
     brakeParkingMode = false;
     brakeTarget = -1;
      Serial.println("Parking brake releasing...");

     // Optional: small retract pulse to ensure release
      digitalWrite(PIN_BRAKE_EXTEND, LOW);
     digitalWrite(PIN_BRAKE_RETRACT, HIGH);
      delay(300);
      digitalWrite(PIN_BRAKE_RETRACT, LOW);
  ...
  if (line == "PARK") {
      bool throttleIdle = (cmdThrottle == 0);
      bool steerIdle    = (cmdSteer == 0);
      bool brakeIdle    = (cmdBrake == 0);

     if (throttleIdle && steerIdle && brakeIdle && !cartInTransit) {
         brakeTarget = 50;
         brakeParkingMode = true;
         Serial.println("Parking brake engaging...");
     } else {
         Serial.println("ERROR: Cannot engage parking brake while cart is moving or in transit.");
      }
     return;
  }
  ...
  if (homingDone) {
     ...
     long sc    = scRaw - steerZeroOffset;
      ...
      if (brakeParkingMode) {
         ...
         // Skip normal brake logic entirely
         goto throttle_section;
      }
      if (cmdBrake) {
